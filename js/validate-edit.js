function validateForm(){
  ...
  taglineNumbersCheck = checkLettersDesc('formCreate','tagline','taglineFeedback')
  ...
  descriptionNumbersCheck = checkLettersDesc('formCreate','description','descriptionFeedback')
...
function checkLettersDesc(formName, inputName, feedbackId){
  // check for latin letters
  let input = document.forms[formName][inputName].value;
  let onlyLetters = /^[a-zA-Z-_\sÅåÄäÖöØøÆæÉéÈèÜüÊêÛûÎî0123456789!?@#$%^&*.,]*$/.test(input);
  if (onlyLetters == false) {
    document.getElementById(feedbackId).innerHTML = "Bara de vanligaste tecknen tillåtna";
    return false;
  } else {
